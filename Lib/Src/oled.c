...
// 显存
uint8_t OLED_GRAM[OLED_PAGE][OLED_COLUMN];//显存数组
...
void OLED_NewFrame()
{
  memset(OLED_GRAM, 0, sizeof(OLED_GRAM));//GRAM往后的数据都设置为0，设置数量取GRAM的大小
}
...
void OLED_ShowFrame()
{
  ...
  sendBuffer[0] = 0x40;//表示发送的是显示在屏幕的数据
 ...
 * @param color 颜色
 */
void OLED_SetPixel(uint8_t x, uint8_t y, OLED_ColorMode color)//设置某个像素亮起
{
  if (x >= OLED_COLUMN || y >= OLED_ROW)//判断下x,y的值，防止数字越界  ||表示且
    return;
  if (!color)
  {
    OLED_GRAM[y / 8][x] |= 1 << (y % 8);//  |=表示或
  }
  else
  {
